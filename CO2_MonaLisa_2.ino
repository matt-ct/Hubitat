/*
  ...
  3/1/20 Creating many functions with short pieces of code
          call functions when needing to update LCD's or send data to MonaLisa.
...
const int led_Interval = 3000;  // update time for LED Display
const unsigned long Lisa_Interval = 60000;  // Update time for Hub Data >32000 needs to be long
// 60000 = 1 min
// 600000 = 10 min
// 900000 = 15 min
// 3600000 = 1 hour
...
unsigned long previousLED_Millis = 0;   // will store last time the LED was updated
unsigned long previousLisa_Millis = 0;
...
void setup()
{
  ...
  writeLisa();  // Send Lisa some data right away on startup
}
...
void loop()
{
  if (currentMillis > millis()) {
    currentMillis = 0;    // handles mills overflow!
  }
  ...
  if (currentMillis - previousLED_Millis >= led_Interval) {
    writeLCD();                                              // only calls writeLCD when time
    previousLED_Millis = currentMillis;

  }

  if (currentMillis - previousLisa_Millis >= Lisa_Interval) {              // Lisa_Interval
    writeLisa();                                              // only calls writeLisa when time
    previousLisa_Millis = currentMillis;
  }
  ...
  if (mySerial.available()) {       // Reads the string from the MonaLisa and calls gotCommand to return string and length
    ...
    if (myInputBuffer[myInputBuffer_p] == '\n') {
      ...
      gotCommand(myInputBuffer, myInputBuffer_p);                           // May want this to pass the command back up to the main loop
      myInputBuffer_p = 0;
    }
    else {
      myInputBuffer_p++;
  ...
  // if (airSensor.dataAvailable())   //  asumeing data is ok for now

  /*
    else                                                      // No SCD30 data availble  not useing right now
    {
      lcd.clear();  // Clear the screen
      lcd.setCursor(0, 0); // move cursor to  line "0" and 0 spaces over
...
void writeLCD() {    // function to update the LCD
...
}   // end writeLCD function

void writeLisa() {  // Function to write to Lisa board
...
void gotCommand(char* com, int comlength) {
  ...
  String TESTcom = "on1";
  String TESTstring(com);      //convert char array to string here.
  
  TESTstring.replace(".", ""); // get rid of decimal points in string
  TESTstring.remove(3);
  
  if (TESTstring == TESTcom) {  
    Serial.println("command to send data");
    writeLisa();
  }
  // Can do something else with that info here...  Not using yet
